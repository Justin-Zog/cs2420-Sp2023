...
public class HashTable<E>
{
    ...
    private void incrementTotalFinds() { totalFinds++; }
    private void incrementTotalProbes() { totalProbes++; }
    ...
    {
        incrementTotalFinds();
        incrementTotalProbes();
        int currentPos = myhash( x );
        int offset = personalizedStep(currentPos);
    ...
    private int personalizedStep( int key )
    {
        // Based off the formula 'R - (x % R)'.
        int prime = nextLowestPrime(array.length);
    ...
    /**
     * Internal method to find the next lowest prime number.
     * @param n the starting number (must be positive).
     * @return the closest prime number that is less than n.
     *
     */
    private static int nextLowestPrime( int n )
    {
        n--;
        if (n % 2 == 0) n--;
        if (n < 1) return 1;
